...
class SecurityService extends EventTarget {
  ...
  async loginWithGitHub() {
    try {
      const firebaseResult: UserCredential =
        await FirebaseService.loginWithGitHub();
      const firebaseUser = firebaseResult.user;

      // ✅ Firebase ID Token — lo que el backend espera
      ...
      try {
        ...
        if (backendError.response) {
          ...
          console.log('📡 Status:', backendError.response.status); // ✅ fix
        }
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------------
  // Vincular la cuena a una cuenta de github
    ...
    const response = await axios.post(
      ...
      { firebaseToken },
      {
        headers: {
          ...
          Authorization: `Bearer ${this.getToken()}`, // ✅ usa getToken()
        },
      }
    );
...
// Desvincular la cuena a una cuenta de github
async unlinkGithubAccount(userId: string) {
  ...
  async loginWithGoogle(googleData: any) {
    ...
    try {
      try {
        ...
        console.log(
          '⚠️ Endpoint Google no disponible, intentando método alternativo...',
        );
  ...
  private async handleGoogleFallback(googleData: any) {
    try {
      ...
      throw new Error(
        'No se pudo conectar con el servidor para autenticar con Google',
      );
    }
  }
