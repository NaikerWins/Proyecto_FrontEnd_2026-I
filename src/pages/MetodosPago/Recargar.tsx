...
export default function RecargarMetodoPago() {
  ...
  const apiCargarTarjeta = async (tarjetaId: number) => {
    try {
      const tarjetaData = await metodoPagoService.getById(tarjetaId);
  ...
  const handleIniciarRecarga = async (e: React.FormEvent) => {
    ...
    try {
      const res = await metodoPagoService.iniciarRecarga(
        parseInt(id!),
        parseFloat(monto),
        email
      );
      ...
      setError(
        e.response?.data?.message || "Error al iniciar recarga."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmar = async () => {
    ...
    try {
      await metodoPagoService.confirmarRecarga(
        referencia,
        "Aceptada",
        parseFloat(monto)
      );
      ...
      setError(
        e.response?.data?.message || "Error al confirmar recarga."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      {tarjeta && (
        <div className="mb-6 rounded border border-stroke bg-white p-4 dark:border-strokedark dark:bg-boxdark">
          <p className="text-sm text-gray-500">
            Tipo:
          </p>

          <p className="font-semibold capitalize dark:text-white">
            {tarjeta.metodopago?.tipo}
          </p>
      ...
      >
        ...
        <div className="rounded bg-blue-50 border border-blue-200 px-4 py-3 text-xs text-blue-700">
          ℹ️ Se aplicará un cargo por servicio del
          2.9% + $900
        </div>
