...
const DropdownMessage = () => {
    ...
    const { mensajesNuevos, limpiarMensajesNuevos } = useMensajeriaContext();
    ...
    return (
        <li className="relative" x-data="{ dropdownOpen: false, notifying: true }">
            ...
            >
                ...
                {mensajesNuevos.length > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 z-1 flex h-4 w-4 items-center justify-center rounded-full bg-meta-1 text-[10px] text-white font-bold">
                        {mensajesNuevos.length > 9 ? '9+' : mensajesNuevos.length}
            ...
            >
                <div className="px-4.5 py-3 flex justify-between items-center">
                    <h5 className="text-sm font-medium text-bodydark2">
                        Mensajes {mensajesNuevos.length > 0 && `(${mensajesNuevos.length} nuevos)`}
                    </h5>
                    <button
                ...
                <ul className="flex h-auto flex-col overflow-y-auto">
                    {mensajesNuevos.length === 0 ? (
                        <li className="flex items-center justify-center py-8 text-sm text-bodydark2">
                            No hay mensajes nuevos
                        </li>
                    ) : (
                        mensajesNuevos.slice(0, 5).map((msg, index) => (
                            <li key={index}>
                                <button
                                    className="flex w-full gap-4.5 border-t border-stroke px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4 text-left"
                                    onClick={handleVerTodos}
                                >
                                    {/* Avatar con inicial */}
                                    <div className="flex h-12.5 w-12.5 flex-shrink-0 items-center justify-center rounded-full bg-primary text-white font-bold text-lg">
                                        {msg.emisorNombre.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h6 className="text-sm font-medium text-black dark:text-white truncate">
                                            {msg.emisorNombre}
                                        </h6>
                                        {msg.tipo === 'grupo' && (
                                            <p className="text-xs text-primary">{msg.grupoNombre}</p>
                                        )}
                                        <p className="text-sm truncate">{msg.contenido}</p>
                ...
                <div className="border-t border-stroke">
                    <button
                        onClick={handleVerTodos}
                        className="block w-full py-3 text-center text-sm font-medium text-primary hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4"
                    >
                        Ir a mensajes
                    </button>
                </div>
