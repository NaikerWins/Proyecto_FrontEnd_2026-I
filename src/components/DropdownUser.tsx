...
const DropdownUser = () => {
  ...
  const handleUnlinkGoogle = async () => {
    ...
    const confirm = await Swal.fire({
      ...
      text: 'Necesitarás crear una contraseña para poder entrar con email.',
      icon: 'warning',
    ...
    // Paso 2 — Pide la nueva contraseña
    const { value: password } = await Swal.fire({
      title: 'Crea tu contraseña',
      html: `
            <input id="swal-pass1" type="password" placeholder="Nueva contraseña" 
                class="swal2-input" style="margin-bottom:8px"/>
            <input id="swal-pass2" type="password" placeholder="Confirmar contraseña" 
                class="swal2-input"/>
        `,
      confirmButtonText: 'Guardar y desvincular',
      showCancelButton: true,
      cancelButtonText: 'Cancelar',
      preConfirm: () => {
        const p1 = (document.getElementById('swal-pass1') as HTMLInputElement)
          .value;
        const p2 = (document.getElementById('swal-pass2') as HTMLInputElement)
          .value;
        if (!p1 || p1.length < 6) {
    ...
    try {
      // Paso 3 — Guarda la nueva contraseña en el usuario
      await api.put(`/users/${user.id}`, {
        name: user.name,
        email: user.email,
        password: password, // el backend la encripta con SHA256
      ...
      // Paso 4 — Desvincula Google
      await api.put(`/security/unlink-google/${user.id}`);

      await Swal.fire({
        title: '¡Listo!',
        text: 'Cuenta desvinculada. Inicia sesión con tu email y contraseña.',
        icon: 'success',
        timer: 3000,
        showConfirmButton: false,
      });

      // Paso 5 — Logout para que entre con email y contraseña
      SecurityService.logout();
      setDropdownOpen(false);
      ...
      Swal.fire({
        title: 'Error',
        text: 'No se pudo desvincular la cuenta',
        icon: 'error',
        timer: 2000,
      });
    }
  ...
  const handleLinkGithub = async () => {
    try {
      const result = await SecurityService.linkGithubAccount();
      ...
      Swal.fire({
        title: 'Error',
        text: error.message || 'No se pudo vincular la cuenta de GitHub.',
        icon: 'error',
        confirmButtonText: 'Aceptar',
      });
    }
  ...
  const handleUnlinkGithub = async () => {
    if (!user?.githubAccount) return;

    const confirm = await Swal.fire({
      title: '¿Desvincular GitHub?',
      text: 'Ya no podrás iniciar sesión con GitHub.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#3085d6',
    ...
    try {
      await SecurityService.unlinkGithubAccount(user.id!);
      ...
      await Swal.fire({
        title: '¡Desvinculada!',
        text: 'Tu cuenta de GitHub ha sido desvinculada.',
        icon: 'success',
        confirmButtonText: 'Aceptar',
      });
      ...
      Swal.fire({
        title: 'Error',
        text: error.message || 'No se pudo desvincular la cuenta de GitHub.',
        icon: 'error',
        confirmButtonText: 'Aceptar',
      });
    }
      ...
      >
        ...
        {user?.googleAccount && (
          <button
            ...
            className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-red-500 lg:text-base cursor-pointer w-full border-t border-stroke dark:border-strokedark"
          >
            <svg
              width="22"
              height="22"
        ...
        {user?.githubAccount ? (
          ...
          <button
            ...
            className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base cursor-pointer w-full border-t border-stroke dark:border-strokedark"
          >
            <svg
              height="22"
              width="22"
